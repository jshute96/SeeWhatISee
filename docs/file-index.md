...
| `src/capture-page/drawing.ts` | Capture-page highlight overlay — `initDrawing(ctx)`: edits / history / polyline / boxDrag state, snap-to (incl. `panSnapRects` for pan snap), render, drawViewportEdges, Shrink + View cropped + Convert last box + annotation Copy / Paste / Import (More menu), tool palette; bake helpers (`hasBakeableEdits`, `editFlags`, `activeCrop`, `arrowBarbs`, `pctRectToPixels`) and `__seeState` hooks exported for main |
...
| `tests/e2e/capture-drawing-convert.spec.ts` | E2E for the Convert last drawn box action — enable rule, the kind submenu, geometry-preserving conversion, Undo, keyboard reach |
