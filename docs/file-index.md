...
| `src/capture.ts` | Capture functions (`captureVisible`, `savePageContents`, `captureBothToMemory`, `downloadScreenshot`/`downloadHtml`/`waitForDownloadComplete`, `recordDetailedCapture`, `clearCaptureLog`) and `log.json` sidecar writing |
