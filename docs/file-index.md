...
| `src/background.ts` | MV3 service worker — handles toolbar click + right-click context menu (immediate / 2s / 5s delayed captures, save html contents) and exposes `self.SeeWhatISee` for tests |
| `src/capture.ts` | Capture functions (`captureVisible`, `savePageContents`), metadata sidecar writing (`latest.json`, `log.json`), and the storage-backed capture log |
