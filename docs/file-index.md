...
| `src/shared-styles.css` | Page-wide `.btn` chrome + `.app-header` / `.app-footer` light-purple bar (matches the toolbar icon) shared by `capture.html` and `options.html` |
...
| `tests/e2e/jpg-png-cache-edit-sync.spec.ts` | E2E regressions — JPG source: cache-hit `.png` ext stable across repeat copies; undo-all reverts to `.jpg`; rebump same-revision no-op so multi-Capture log keeps `.png` |
