...
| `src/capture.html` | Extension page for the "Capture with details…" flow (URL, HTML size, save options + Copy/Edit buttons, edit-HTML modal, prompt, highlight overlay) |
| `src/capture-page.ts` | Controller script for `capture.html`: data fetch, prompt/textarea behavior, Copy-filename clipboard writes, HTML edit dialog, SVG highlight overlay, canvas bake-in on save, image fit-to-viewport |
