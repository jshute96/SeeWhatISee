...
function renderWithoutTable(data: OptionsData): void {
  ...
  //   1. Undelayed actions (the everyday picks).
  //   2. A single combined "Capture after delay" section holding every
  //      delaySec > 0 entry (only `capture-Ns` and `save-screenshot-Ns`
  //      today; all other bases are `supportsDelayed: false`).
  const immediate: OptionsActionRow[] = [];
  const delayed: OptionsActionRow[] = [];
  for (const a of data.actions) {
    if (a.isSelection) continue;
    if (a.delaySec > 0) delayed.push(a);
  ...
  if (delayed.length) {
    // The delayed group is short (today: capture-3s + save-screenshot-3s)
    // so we render it as a plain section row alongside the immediate
    // rows — no expand/collapse toggle needed.
    tbody.appendChild(sectionRow('Capture after delay', WITHOUT_TABLE_COLS));
    for (const a of delayed) appendWithoutRow(tbody, a, data);
  }
}
