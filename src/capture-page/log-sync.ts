...
export function showLogSyncDialog(prompt: LogSyncPrompt, h: LogSyncHandlers, attempt = 0): void {
  ...
  // Option A: the corrupt-file reason asks for the file to be fixed
  // before the Retry; an unreadable file has no step we can name, so
  // it collapses to the bare Retry button. The rest of the list is
  // static markup.
  const corrupt = prompt.reason === 'corrupt-file';
  fixFileEl.hidden = !corrupt;
  stepFileEl.hidden = !corrupt;
  optionAEl.classList.toggle('log-sync-retry-only', !corrupt);
