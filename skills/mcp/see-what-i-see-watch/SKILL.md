...
2. Each `notifications/resources/updated` notification means at least one new capture arrived. Read `seewhatisee://captures/stream?after=<timestamp>` to read **all records** that follow that one — it returns `{ records: [...] }` in order. Process each, then remember the last record's `timestamp` as the new cursor.
