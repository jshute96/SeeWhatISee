...
3. Process the capture:
   ...
   * **If `prompt` is present, treat it as the user's instruction for this
     capture and act on it directly.** Use the screenshot, HTML, and/or `url`
     as the subject of that instruction. If no screenshot or HTML was saved,
     the `url` is what the prompt is about.
   * If `prompt` is absent:
     ...
     - For URL-only captures (no screenshot, no HTML), report the `url` and
