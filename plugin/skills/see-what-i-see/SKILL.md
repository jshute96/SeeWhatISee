...
2. The JSON record contains `{timestamp, url}` plus any of:
  - `screenshot` — object `{ "filename": "<abs path>", "hasHighlights": true? }` pointing at a PNG file. `hasHighlights: true` means the user drew red markup (boxes and/or lines) on top of the screenshot to call attention to specific regions.
