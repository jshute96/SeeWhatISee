...
* **The Capture page's tool palette works from the keyboard.** `Tab` to a drawing tool and `Space` / `Enter` now selects it (it used to highlight but not stick). The *Zoom…*, *More…* and *Ask* menus are arrow-navigable: `↓` / `↑` step through the items and wrap, `Home` / `End` jump to the ends, `Enter` picks, and `Esc` closes. Opening a menu with the keyboard starts on its first item; opening it with the mouse highlights nothing until the first arrow press.
