...
* **Smooth zoom.** `Ctrl + mouse wheel` and trackpad pinch now zoom by a smooth amount instead of jumping between fixed levels, so it can be set to whatever size suits the screenshot. The Zoom button shows a percentage when the zoom isn't at one of the menu's presets, and pinch zoom works properly on trackpads (notably Chromebooks, where it used to skip several levels per gesture or not respond at all). The 1× / 2× / 4× / 8× menu items remain as one-click presets, and `Alt +` / `Alt −` step by one wheel detent's worth.
