...
* Restore last capture — More-menu entry that re-opens the most recently closed Capture page with prompt, save checkboxes, drawing edits + undo stack, selected tool, and zoom rehydrated. Saved on every close (Capture / Ask / manual); dropped on the next new capture (or for quota relief on a tight Capture / Ask write).
